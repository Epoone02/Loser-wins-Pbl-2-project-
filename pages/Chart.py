...
if os.path.exists(csv_path):
    ...
    st.info(f"CSV not found at `{csv_path}`. Put your CSV files in a `data/` folder next to the app.")
...
if run_btn or "sim_results" not in st.session_state:
    with st.spinner(f"Running {num_rounds} rounds…"):
        st.session_state["sim_results"] = cached_simulation(num_rounds, num_players, max_price, base_cost, alpha)
